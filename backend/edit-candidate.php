                                    ...
                                    <div class="col-md-8 col-md-offset-2">
                                        <div class="panel">
                                            <div class="panel-heading">
                                                <div class="panel-title">
                                                    <h5 align = "center">Edit Candidate</h5>
                                                </div>
                                            </div>
												   <?php if($msg){?>
                                        ...
                                        <?php } ?>
                                            ...
                                            <div class="panel-body">
												...
												<?php 
$stid = intval(icrypt($_GET['stid'],'d'));
$sql = "SELECT `Name`,indexNo,gender,contactNo,PositionID,ballotNo,photo,positionName FROM tblcandidate INNER JOIN tblposition ON PositionID = id WHERE indexNo=:stid";
$query = $dbh->prepare($sql);
$query->bindParam(':stid',$stid,PDO::PARAM_STR);
$query->execute();
...
{  ?>
												...
												<div class="col-sm-7">
												<input type="text" name="indexno" value="<?php echo htmlentities($row->indexNo); ?>" class="form-control" id="indexno" maxlength="15" required="required" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" autocomplete="off" readonly>
												</div>
												</div>
												<div class="form-group">
												...
												<option value="<?php echo htmlentities($row->PositionID); ?>"><?php echo htmlentities($row->positionName); ?></option>
												<?php $sql1 = "SELECT id,positionName from tblposition";
													...
													<label for="default" class="col-sm-3 control-label">Ballot Number</label>
													   <div class="col-sm-7">
														 <select name="ballot" class="form-control term" id="ballot" required="required" >
															<option value="<?php echo htmlentities($row->ballotNo); ?>"><?php echo htmlentities($row->ballotNo); ?></option>
												...
												<div class="col-sm-7">
												<input type="text" name="phoneno" value="<?php echo htmlentities($row->contactNo); ?>" class="form-control" id="phoneno" maxlength="15" required="required" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" autocomplete="off">
												</div>
												</div>
<?php }} ?>
			<div class="form-group">
